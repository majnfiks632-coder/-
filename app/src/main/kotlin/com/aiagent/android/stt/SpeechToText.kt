 ...
 *  - "android" (default): the on-device [SpeechRecognizer] — no external service required.
 *  - "openai" / "groq" / any OpenAI-compatible: POST the recorded WAV to the active provider's
 *    `/audio/transcriptions` endpoint. Works with OpenAI, Groq, self-hosted Whisper, etc.
 */
class SpeechToText(private val context: Context, private val settings: Settings) {
    ...
    suspend fun transcribeFile(file: File, language: String? = null): String {
        return when (settings.sttProvider) {
            ...
            else -> transcribeWithProvider(file, language)
        }
    }
    ...
    private suspend fun transcribeWithProvider(file: File, language: String?): String {
        if (!file.exists()) return "[файл не найден: ${file.absolutePath}]"
        val key = settings.apiKey
        if (key.isBlank()) return "[нужен API-ключ активного провайдера в настройках]"
        val baseUrl = settings.baseUrl.trimEnd('/')
        if (baseUrl.isBlank()) return "[нужен Base URL активного провайдера в настройках]"
        ...
        return try {
            ...
            Log.w(TAG, "whisper transcription failed", e)
